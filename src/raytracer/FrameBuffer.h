...
__host__ inline FrameBuffer::FrameBuffer(unsigned int width, unsigned int height) : width{ width }, height{ height } {}

__device__ inline void FrameBuffer::writePixel(int x, int y, glm::vec4 pixel) {
	int idx = y * width + x;
	// convert RGBA to BGRA that buffer uses
	device_ptr[idx] = glm::packUnorm4x8(glm::vec4(pixel.b, pixel.g, pixel.r, pixel.a));
}

__device__ inline glm::vec3 FrameBuffer::color(const Ray& r, Hittable* world, curandState* local_rand_state) {
	Ray cur_ray = r;
	...
	for (int i = 0; i < 50; i++) {
		...
		if (world->hit(cur_ray, 0.001f, FLT_MAX, rec)) {
			...
			// back ground color interpolation
