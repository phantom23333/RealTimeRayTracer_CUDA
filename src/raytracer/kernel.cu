...
__global__ void create_world(thrust::device_ptr<World*> d_world, thrust::device_ptr<Camera*> d_camera, CameraInfo camera_info) {
	if (threadIdx.x == 0 && blockIdx.x == 0) {
		*d_world = new World();
		...
		device_world->add(new Sphere(glm::vec3(1, 0, -1), 0.5f, new Diffuse(glm::vec3(0.725f, 0.71f, 0.68f))));
		//device_world->add(new Sphere(glm::vec3(0, -100.5, -1), 100.0f, new Lambertian(glm::vec3(0.8f, 0.8f, 0.0f))));
		device_world->add(new Sphere(glm::vec3(-1, 0, -1), 0.5f, new Dielectric(2.4f)));
		device_world->add(new Sphere(glm::vec3(0, 0, -1), 0.5f, new Glossy(glm::vec3(1.0f, 0.8f, 0.0f), 0.1f)));
		device_world->add(new Sphere(glm::vec3(0, 0, 1.5f), 0.5f, new Glossy(glm::vec3(0.63f, 0.065f, 0.15f), 0.5f)));
		//device_world->add(new Sphere(glm::vec3(2, 0, -1), 0.5f, new MicroFacet( 1.0f,1.5f, glm::vec3(0.725f, 0.71f, 0.68f))));

		device_world->add(new Sphere(glm::vec3(0, -1000.5, 0), 1000.f, 
			new Diffuse(
				new CheckerTexture(
